import type { HistorialRecord, GuardarResult } from '../models/despacho.model'
...
export function generar(data: GuardarResult): void {
  ...
  const filas = items
    .map((i: Pedido, idx: number) => {
      ...
      return '<tr class="' + sepClass + '"' + rowBg + '>' +
        ...
        '<td style="white-space:nowrap">' + esc(i.pedido) + '</td>' +
        '<td style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + esc(i.cliente.substring(0, 30)) + '</td>' +
        '<td contenteditable="true" style="background:#fffde7;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + esc(i.agencia) + '</td>' +
        '<td contenteditable="true" style="white-space:nowrap">' + esc(i.codigoVenta || '-') + '</td>' +
        '<td contenteditable="true" style="white-space:normal;word-break:break-word">' + esc(i.producto) + '</td>' +
  ...
  let filasObs = ''
  pedidosUnicos.forEach((ped: string) => {
    const itemsPedido = items.filter((i: Pedido) => i.pedido === ped)
    const obs = itemsPedido[0]?.observacion || ''
    if (obs.trim() !== '') {
      filasObs += '<tr>' +
        '<td style="font-weight:bold">' + esc(ped) + '</td>' +
        '<td>' + esc(itemsPedido[0]?.cliente || '') + '</td>' +
        '<td>' + esc(itemsPedido[0]?.agencia || '') + '</td>' +
        '<td>' + esc(obs) + '</td>' +
        '</tr>'
    }
  })
  ...
  const html = '<!DOCTYPE html>' +
    ...
    '<head>' +
    '<meta charset="UTF-8">' +
    '<title>Despacho ' + esc(data.id) + '</title>' +
    '<style>' +
    ...
    '<body>' +
    '<div class="save-bar">' +
    '<span><strong>' + esc(data.id) + '</strong></span>' +
    '<button onclick="guardarCambios()">Guardar cambios</button>' +
    '<span class="save-status" id="saveStatus"></span>' +
    '</div>' +
    '<div class="header">' +
    '<h1>FORLI &#8212; HOJA DE DESPACHO</h1>' +
    '<div class="sub">ID: ' + esc(data.id) + ' | Emitido: ' + esc(data.fecha) + '</div>' +
    '</div>' +
    '<table class="info-table">' +
    '<tr>' +
    '<td>CHOFER: ' + esc(data.chofer) + '</td>' +
    '<td>PLACA: ' + esc(data.placa) + '</td>' +
    '<td>PEDIDOS: ' + totalPedidos + '</td>' +
    '</tr>' +
    '<tr>' +
    '<td>ITEMS: ' + items.length + '</td>' +
    '<td colspan="2">FECHA: ' + esc(data.fecha) + '</td>' +
    '</tr>' +
    '</table>' +
...
export function reimprimir(data: HistorialRecord): void {
