...
import type { CampoFecha, Filtros } from '../stores/filtros.store'
...
export function agruparPorUbigeo(pedidos: Pedido[], campoFecha: CampoFecha = 'PEDIDOFECHA'): UbigeoGroup[] {
  ...
  for (const p of pedidos) {
    ...
    if (!ub) { ub = new Map(); byUbigeo.set(p.ubigeo, ub) }
    let ag = ub.get(p.agencia)
    if (!ag) { ag = new Map(); ub.set(p.agencia, ag) }
    let cli = ag.get(p.cliente)
    if (!cli) { cli = new Map(); ag.set(p.cliente, cli) }
    let ped = cli.get(p.pedido)
    if (!ped) { ped = []; cli.set(p.pedido, ped) }
    ped.push(p)
  }
